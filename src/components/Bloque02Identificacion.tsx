...
export const Bloque02Identificacion: React.FC = () => {
  ...
  const [isGeneratingAllAI, setIsGeneratingAllAI] = useState<boolean>(false);
  ...
  const handleGenerateAllAI = async () => {
    ...
    setNotification('Generando y completando técnicamente los 10 peligros con IA Generativa según GTC 45...');
    try {
      await generateMatrixWithAI();
      setNotification('¡10 Peligros generados y contextualizados con IA exitosamente!');
      setTimeout(() => setNotification(null), 5000);
    } catch (err: any) {
      setNotification(`Error al generar con IA: ${err?.message || 'Verifique la conexión o el servidor.'}`);
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            id="btn-generar-matriz-ia"
            title="Generar y estructurar los 10 peligros oficiales con IA Generativa según GTC 45"
            className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 disabled:opacity-50 text-white text-xs font-bold rounded-lg shadow-xs transition-all cursor-pointer"
          >
            {isGeneratingAllAI ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4 text-amber-300" />}
            <span>{isGeneratingAllAI ? 'Generando con IA...' : 'Generar Textos con IA'}</span>
          </button>
          ...
          >
            ...
            <span>Cargar 10 Peligros ({emgCount}/10)</span>
          </button>

          <button
            onClick={handleNew}
            id="btn-identificar-nuevo-peligro"
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors cursor-pointer"
          >
            <Plus className="w-4 h-4" />
            <span>+ Identificar Nuevo</span>
          </button>
        </div>
      </div>
